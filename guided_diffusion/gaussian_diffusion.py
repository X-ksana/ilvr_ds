...
class GaussianDiffusion:
    ...
    def p_sample_loop_progressive(
        ...
        # Define shapes for image and mask components
        shape_u = (shape[0], 3, shape[2], shape[3])  # Shape for image (3 channels)
        shape_d = (shape[0], 3, int(shape[2] / D), int(shape[3] / D))  # Downsampled shape
        shape_mask = (shape[0], 3, shape[2], shape[3])  # Shape for mask (3 channels)
        ...
        for i in indices:
            ...
            # Split the output into image and mask predictions
            image_pred = out["pred_xstart"][:, :3]  # First 3 channels are image
            mask_pred = out["pred_xstart"][:, 3:]   # Remaining channels are mask

            # Calculate image difference and gradient
            if "ref_img" in model_kwargs:
                # Calculate difference between reference and predicted image
                difference = resize(
                    resize(model_kwargs["ref_img"], scale_factors=1.0/D, out_shape=shape_d),
                    scale_factors=D,
                    out_shape=shape_u
                ) - resize(
                    resize(image_pred, scale_factors=1.0/D, out_shape=shape_d),
                    scale_factors=D,
                    out_shape=shape_u
                )
                
                # Calculate norm and gradient for image guidance
                norm = th.linalg.norm(difference)
                norm_grad = th.autograd.grad(outputs=norm, inputs=img)[0]
                
                # Apply guidance to both image and mask components
                # Scale the gradient for image part
                image_grad = norm_grad[:, :3] * scale
                # Scale the gradient for mask part (using a smaller scale to maintain mask structure)
                mask_grad = norm_grad[:, 3:] * (scale * 0.5)  # Reduced scale for mask
                
                # Update the sample with combined gradients
                out["sample"] = out["sample"] - th.cat([image_grad, mask_grad], dim=1)
    ...
    def training_losses(self, model, x_start, t, model_kwargs=None, noise=None):
        ...
        :param x_start: the [N x C x ...] tensor of inputs, where C includes both image and mask channels.
        ...
        # Split input into image and mask components
        # For cardiac MRI: first 1 channel is image, remaining channels are mask
        if x_start.shape[1] > 1:  # We have both image and mask
            image_start = x_start[:, :1]  # First channel is image
            mask_start = x_start[:, 1:]   # Remaining channels are mask
        else:  # Only image data
            image_start = x_start
            mask_start = None
        
        # Add noise to both image and mask components
